...
function defineVersionPlugin(version) {
	...
	return {
		name: "define-version",
		transform(code, id) {
			if (code.includes("__COLLAB_API_VERSION__")) {
				return { code: code.replaceAll("__COLLAB_API_VERSION__", replacement) };
			}
		},
	};
}
...
function getWorkspacePackages() {
	...
	return packagePaths
		...
		.filter((pkg) => pkg.package.exports && !["@collab-api/docs", "@collab-api/demo"].includes(pkg.package.name));
}
