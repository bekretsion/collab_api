# Collab API v4.0 Release Notes

Collab API v4 is a major release that brings cross-runtime support, improved type safety, and important bug fixes. This release focuses on making Collab API run beyond Node.js -- on Bun, Deno, Cloudflare Workers, and Node with uWebSockets -- while improving the developer experience with generic Context typing and structured transaction origins.
...
Collab API is no longer tied to the Node.js `ws` library. The server now uses [crossws](https://github.com/unjs/crossws), a universal WebSocket adapter, enabling Collab API to run on:
...
The built-in `Server` class continues to work as before for Node.js users. For other runtimes, use `Collab API` directly with `handleConnection()`, which now accepts any `WebSocketLike` object and a web-standard `Request`.
...
- **Generic `Context` type parameter** on `Collab API`, `Server`, `Extension`, `Connection`, `ClientConnection`, `DirectConnection`, and all hook payloads *(non-breaking -- defaults to `any`)*
...
- **`SkipFurtherHooksError`** -- extensions can throw `SkipFurtherHooksError` (from `@collab-api/common`) in `onStoreDocument` to signal that persistence was handled and remaining hooks should be skipped *(non-breaking)*
...
- **`ws` package types removed** -- the provider no longer imports `Event`, `MessageEvent`, or `CloseEvent` from the `ws` package. It uses web-standard types and types from `@collab-api/common` instead *(breaking for TypeScript users who relied on `ws` types being re-exported)*
...
- **Attach collision detection** -- `Collab APIProviderWebsocket.attach()` now throws an error if you try to attach two authenticated providers with the same effective name. Previously it silently overwrote the existing provider *(non-breaking for correct usage; may surface existing bugs)*
...
- **Node.js requirement**: `>=22` (specified in `@collab-api/server`)
...
```bash
# Install v4
npm install @collab-api/server@^4.0.0 @collab-api/provider@^4.0.0
```
...
npm install @collab-api/extension-sqlite@^4.0.0 better-sqlite3
...
```typescript
import type { WebSocketLike } from '@collab-api/server';
...
async onChange({ transactionOrigin }) {
  if (transactionOrigin === '__collab-api__redis__origin__') {
    // came from Redis
  }
...
```typescript
import { isTransactionOrigin, shouldSkipStoreHooks } from '@collab-api/server';
...
2. No changes to your Collab API configuration -- the extension API is the same
...
wss.on('connection', (ws: WebSocket, request: IncomingMessage) => {
  collab-api.handleConnection(ws, request, context);
});
```
...
wss.on('connection', (ws, request: Request) => {
  const clientConnection = collab-api.handleConnection(ws, request, context);
...
The provider no longer imports or re-exports `Event`, `MessageEvent`, or `CloseEvent` from the `ws` package. If your TypeScript code relied on these types being available through the provider, import them from `@collab-api/common` or use web-standard types instead.
...
- [ ] Update all `@collab-api/*` packages to v4
...
- [ ] Replace `WebSocket` type imports from `ws` with `WebSocketLike` from `@collab-api/server`
...
### Provider
- [ ] Update `@collab-api/provider` to v4
