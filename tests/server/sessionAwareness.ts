import { parseRoutingKey } from "@collab-api/common";
import type { onAuthenticatePayload, connectedPayload } from "@collab-api/server";
...
test("sessionAwareness: two providers with same doc name both connect successfully", async (t) => {
	await new Promise(async (resolve) => {
		...
		const provider1 = newCollabApiProvider(
			t,
			server,
			{
		...
		const provider2 = newCollabApiProvider(
			t,
			server,
			{
				websocketProvider: socket,
...
test("sessionAwareness: auth failure isolation - provider A fails, provider B succeeds", async (t) => {
	...
	const providerFail = newCollabApiProvider(
		t,
		server,
		{
			websocketProvider: socket,
	...
	const providerOK = newCollabApiProvider(
		t,
		server,
		{
			websocketProvider: socket,
