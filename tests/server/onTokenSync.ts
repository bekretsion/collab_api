...
import type { onAuthenticatePayload, onTokenSyncPayload } from '@collab-api/server'
import { Unauthorized } from '@collab-api/common'
import {
  ...
  await new Promise(async resolve => {
    const server = await newCollabApi(t, {
      ...
      async onTokenSync({ context, connection, documentName, token }: onTokenSyncPayload) {
        ...
        t.truthy(connection)
        t.is(documentName, 'collab-api-test')
        t.is(token, 'SUPER-SECRET-TOKEN')
        resolve('done')
      },
    })
...
test('server requestToken: executes onTokenSync when server requests token after authentication', async t => {
  await new Promise(async resolve => {
    ...
    const document = server.documents.get('collab-api-test')
    if (document) {
