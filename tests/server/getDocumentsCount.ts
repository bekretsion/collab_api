...
import {
  newCollabApi, newCollabApiProvider, randomInteger,
} from '../utils/index.ts'
...
test('the same document name counts as one document', async t => {
  ...
  const providers = [
    newCollabApiProvider(t, server, { name: 'foobar' }),
    newCollabApiProvider(t, server, { name: 'foobar' }),
  ]
...
test('adds and removes different documents properly', async t => {
  ...
  const providers = [
    newCollabApiProvider(t, server, { name: 'foo-1' }),
    newCollabApiProvider(t, server, { name: 'foo-2' }),
    newCollabApiProvider(t, server, { name: 'foo-3' }),
    newCollabApiProvider(t, server, { name: 'foo-4' }),
    newCollabApiProvider(t, server, { name: 'foo-5' }),
  ]
...
test('adds and removes random number of documents properly', async t => {
  ...
  for (let index = 0; index < numberOfProviders; index += 1) {
    providers.push(
      newCollabApiProvider(t, server, { name: `foobar-${index}` }),
    )
  }
  await retryableAssertion(t, tt => {
