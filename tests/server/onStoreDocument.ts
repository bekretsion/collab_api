import type { Extension, onLoadDocumentPayload, onStoreDocumentPayload } from "@collab-api/server";
...
test("doesn’t remove the document from memory when there’s a new connection established during onStoreDocument is called", async (t) => {
	await new Promise(async (resolve) => {
		...
		const socket = newCollabApiProviderWebsocket(t, server);
		const anotherSocket = newCollabApiProviderWebsocket(t, server);
...
test("does not start a new onStoreDocument if there is already one running (should wait for the first one to finish)", async (t) => {
  ...
  */
	...
	await new Promise(async (resolve) => {
		...
		const socket1 = newCollabApiProviderWebsocket(t, server);
