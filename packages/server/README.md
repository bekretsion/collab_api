...
[![Downloads](https://img.shields.io/npm/dm/@collab-api/server.svg)](https://npmcharts.com/compare/tiptap?minimal=true)
...
For a database-backed server, combine with an extension like [`@collab-api/extension-sqlite`](../extension-sqlite) or [`@collab-api/extension-database`](../extension-database).

## Non-Node.js runtimes

Use the `Collab API` class directly to attach to any `WebSocketLike` instance (Bun, Deno, Cloudflare Workers, Express, etc.):

```js
import { CollabApi } from "@collab-api/server"

const collab-api = new CollabApi({ /* config */ })
...
Full reference, hooks, extensions, and scaling guides: [tiptap.dev/docs/collab-api](https://tiptap.dev/docs/collab-api).
