...
import {
	...
	Unauthorized,
	WsReadyStates,
} from "@collab-api/common";
import * as decoding from "lib0/decoding";
import Connection from "./Connection.ts";
import type Document from "./Document.ts";
import type { CollabApi } from "./CollabApi.ts";
 ...
 *
 * TODO-refactor:
 * - use event handlers instead of calling hooks directly, hooks should probably be called from CollabApi.ts
 */
export class ClientConnection<Context = any> {
	...
	constructor(
		...
		private readonly documentProvider: {
			createDocument: CollabApi["createDocument"];
		},
		// TODO: change to events
		private readonly hooks: CollabApi["hooks"],
	...
	private createConnection(
		...
		instance.onClose(async (document, event) => {
			...
			const disconnectHookPayload: onDisconnectPayload = {
				instance: this.documentProvider as CollabApi, // TODO, this will be removed when we use events instead of hooks for this class
		...
		try {
			...
			if (isFirst) {
				...
				this.hookPayloads[rawKey] = {
					instance: this.documentProvider as CollabApi,
