import { type CloseEvent, readAuthMessage } from "@collab-api/common";
...
export class MessageReceiver {
	...
	public apply(provider: CollabApiProvider, emitSynced: boolean) {
		const { message } = this;
	...
	private applySyncMessage(provider: CollabApiProvider, emitSynced: boolean) {
		const { message } = this;
	...
	applySyncStatusMessage(provider: CollabApiProvider, applied: boolean) {
		if (applied) {
			provider.decrementUnsyncedChanges();
		}
	}

	private applyAwarenessMessage(provider: CollabApiProvider) {
	...
	private applyAuthMessage(provider: CollabApiProvider) {
		const { message } = this;
	...
	private applyQueryAwarenessMessage(provider: CollabApiProvider) {
		if (!provider.awareness) return;
