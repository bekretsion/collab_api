import { WsReadyStates } from "@collab-api/common";
import { retry } from "@lifeomic/attempt";
import { createDecoder, readVarString, readVarUint } from "lib0/decoding";
import * as encoding from "lib0/encoding";
import * as time from "lib0/time";
import EventEmitter from "./EventEmitter.ts";
import type { CollabApiProvider } from "./CollabApiProvider.ts";
...
export type CollabApiWebSocket = WebSocket & { identifier: string };
export type HocusPocusWebSocket = CollabApiWebSocket;

export type CollabApiProviderWebsocketConfiguration = Required<
	Pick<CompleteCollabApiProviderWebsocketConfiguration, "url">
> &
	Partial<CompleteCollabApiProviderWebsocketConfiguration>;

export interface CompleteCollabApiProviderWebsocketConfiguration {
	/**
	 ...
	 */
	providerMap: Map<string, CollabApiProvider>;
}

export class CollabApiProviderWebsocket extends EventEmitter {
	...
	public configuration: CompleteCollabApiProviderWebsocketConfiguration = {
		url: "",
		autoConnect: true,
	...
	constructor(configuration: CollabApiProviderWebsocketConfiguration) {
		super();
	...
	attach(provider: CollabApiProvider) {
	...
	detach(provider: CollabApiProvider) {
		const key = provider.effectiveName;
	...
	public setConfiguration(
		configuration: Partial<CollabApiProviderWebsocketConfiguration> = {},
	): void {
	...
	private addToQueue(message: any) {
		if (message instanceof Uint8Array) {
			...
			if (
				parsed &&
				CollabApiProviderWebsocket.DEDUPLICATABLE_TYPES.has(parsed.messageType)
			) {
