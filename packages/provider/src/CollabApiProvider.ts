import { awarenessStatesToArray, makeRoutingKey, parseRoutingKey } from "@collab-api/common";
import { Awareness, removeAwarenessStates } from "y-protocols/awareness";
import * as Y from "yjs";
import EventEmitter from "./EventEmitter.ts";
import type { CompleteCollabApiProviderWebsocketConfiguration } from "./CollabApiProviderWebsocket.ts";
import { CollabApiProviderWebsocket } from "./CollabApiProviderWebsocket.ts";
...
export type CollabApiProviderConfiguration = Required<
	Pick<CompleteCollabApiProviderConfiguration, "name">
> &
	Partial<CompleteCollabApiProviderConfiguration> &
	(
		| (Required<Pick<CompleteCollabApiProviderWebsocketConfiguration, "url">> &
				Partial<
					Pick<
						CompleteCollabApiProviderWebsocketConfiguration,
						"preserveTrailingSlash"
					>
				>)
		| Required<
				Pick<CompleteCollabApiProviderConfiguration, "websocketProvider">
		  >
	);

export interface CompleteCollabApiProviderConfiguration {
	/**
	 ...
	 * socket connection, or ensure that the Provider receives messages before running into `CollabApiProviderWebsocket.messageReconnectTimeout`.
	 */
...
export class CollabApiProvider extends EventEmitter {
	public configuration: CompleteCollabApiProviderConfiguration = {
		name: "",
		// @ts-expect-error
		document: undefined,
		// @ts-expect-error
	...
	constructor(configuration: CollabApiProviderConfiguration) {
		super();
	...
	public setConfiguration(
		configuration: Partial<CollabApiProviderConfiguration> = {},
	): void {
		if (!configuration.websocketProvider) {
			this.manageSocket = true;
			this.configuration.websocketProvider = new CollabApiProviderWebsocket(
				configuration as CompleteCollabApiProviderWebsocketConfiguration,
			);
		}
	...
	async connect() {
		...
		console.warn(
			"CollabApiProvider::connect() is deprecated and does not do anything. Please connect/disconnect on the websocketProvider, or attach/deattach providers.",
		);
	}
	...
	disconnect() {
		...
		console.warn(
			"CollabApiProvider::disconnect() is deprecated and does not do anything. Please connect/disconnect on the websocketProvider, or attach/deattach providers.",
		);
	}
	...
	attach() {
		...
		// Mark attached BEFORE websocketProvider.attach() — that call may
		// synchronously invoke this.onOpen() if the WS is already open, which
		// will call send(); send() short-circuits when _isAttached is false.
